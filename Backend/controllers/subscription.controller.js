...
const getUserChannelSubscribers = asyncHandler(async (req, res) => {
    ...
    const userChannelSubscribers = await Subscription.aggregate([
      {
        ...
        },
        {
          $project: {
              _id: 1,
              SubscriberID : `$channelSubscribers._id`,
              SubscriberAvatar : `$channelSubscribers.avatar`,
  ...
  if(!userChannelSubscribers.length){
    ...
    return res.status(201).json(
         new APIResponse(200,userChannelSubscribers,"User has no Subscriber"))
    }
...
const getSubscribedChannels = asyncHandler(async (req, res) => {
    ...
    const userSubscribedChannels = await Subscription.aggregate([
        ...
        },
        {
            $project: {
                _id : 0,
                SubscriberID : `$subscribedChannels._id`,
                SubscribersAvatar :'$subscribedChannels.avatar',
  ...
  if(!userSubscribedChannels.length){
   console.log("No Channel Subscribed")
    return res.status(201).json(
         new APIResponse(200,userSubscribedChannels,"No Channel Subscribed"))
    }
